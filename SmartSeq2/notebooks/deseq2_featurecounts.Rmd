...
sample_info <- sample_info[!rownames(sample_info) %in% exclude, ]
...
comparison_info <- read_excel("Comparisons.xlsx", "Kevin")
sample_info <- merge(sample_info, comparison_info, all.x = TRUE, sort=FALSE)
rownames(sample_info) <- sample_info$sample_id
...
for(contrast in names(contrasts))
{
 ...
 samples <- sample_info$sample_id[sample_info[, contrasts[[contrast]][1]] %in% contrasts[[contrast]][2:3]]
  ...
  dds <- setup_dds(counts=round(data),columnData=sample_info,
                   subset=samples,
                   design = sprintf("~ %s", contrasts[[contrast]][1]),
                   reference_levels=reference_levels)
  
  message("A:", sum(dds[[contrasts[[contrast]][1]]] == "A"))
  message("B:", sum(dds[[contrasts[[contrast]][1]]] == "B"))
