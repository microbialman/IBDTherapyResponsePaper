...
#correlate MEs with one another
betMEcors = cor(MEs)
...
filtadjCorClean2=filtadjCor[-nonsigarows2,]
filtadjPClean2=filtadjP[-nonsigarows2,]
rownames(filtadjCorClean2)=gsub("ME","",rownames(filtadjCorClean2))
rownames(filtadjPClean2)=rownames(filtadjCorClean2)

simpHeat(t(filtadjCorClean2),t(filtadjPClean2),colorer(11))+scale_fill_gradientn(colors=colorer(11),breaks=c(-0.85,0,0.85),labels=c(-0.85,0,0.85),limits=c(-0.85,0.85))+scale_color_manual(values=c("black",NA),labels=c("FDR<0.05",""),name="Significance")
...
#calculate MEs just for IBD patients
IBDMEs = moduleEigengenes(tpm[traitData$Combined==1,], colors = bwModuleColors, excludeGrey = TRUE,)
IBDMEs = IBDMEs$eigengenes
colnames(IBDMEs)=gsub("ME","",colnames(IBDMEs))
colnames(IBDMEs)=namekey[match(colnames(IBDMEs),namekey[,1]),2]

write.csv(IBDMEs,here("Data","Generated","discovery_MEs_IBD_patients_only.csv"))

#correlate MEs vs numerical versions of traits
#remove CRC and control vs ibd columns and CRC samples
IBDnumTraitData = numTraitData[numTraitData$Combined==2,-c(14,17)]
IBDmoduleTraitCor = cor(IBDMEs, IBDnumTraitData, use = "p")
