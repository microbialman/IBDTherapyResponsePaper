...
#load in tpm counts
tpmcounts=read.csv(here("Data","Input","discovery_tpm_counts.csv"))
...
samples2remove=read.csv(here("Data","Input","failed_samples.csv"))
...
traitData = read.csv(here("Data","Input","discovery_metadata.csv"),stringsAsFactors = T)
...
#full heat
P=ifelse(moduleTraitPvalue<0.05,1,0)
cat("**All modules/trait correlations (squares indicate p<0.05)**")
simpHeat(t(moduleTraitCor),t(P),colorer(11))

#filtered heat
filtCor=moduleTraitCor[-nonsigrows,-nonsigcols]
filtP=moduleTraitPvalue[-nonsigrows,-nonsigcols]
filtP=ifelse(filtP<0.05,1,0)
cat("**Modules/traits with at least one nominally significant correlation (p<0.05)**")
simpHeat(t(filtCor),t(filtP),colorer(11))
...
#reduced version 
filtadjCorClean=filtadjCor[,c(10,9,8,4,3,2,15)]
filtadjPClean=filtadjP[,c(10,9,8,4,3,2,15)]
colnames(filtadjCorClean)=c("Anti-TNF","Anti-Integrin","Corticosteroids","Sampling Site","NANCY Score","Inflammation Status","IBD Status")
rownames(filtadjCorClean)=gsub("ME","",rownames(filtadjCorClean))
rownames(filtadjPClean)=rownames(filtadjCorClean)
colnames(filtadjPClean)=colnames(filtadjCorClean)

simpHeat(t(filtadjCorClean),t(filtadjPClean),colorer(11))+scale_fill_gradientn(colors=colorer(11),breaks=c(-0.8,0,0.8),labels=c(-0.8,0,0.8),limits=c(-0.8,0.8))+scale_color_manual(values=c("black",NA),labels=c("FDR<0.05",""),name="Significance")
ggsave(here("Plots","discovery_cleaned_metadata_heatmap.pdf"),width = 15,height = 8)
...
rownames(modcorframe)=substring(rownames(modcorframe),3)
...
```{r echo=F, code=readLines("Scripts/plot_module_networks.R")}
```
