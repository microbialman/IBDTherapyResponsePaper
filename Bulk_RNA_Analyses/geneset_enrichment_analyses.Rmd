...
modules<-read.csv(here("Data","Input","module_gene_mapping.csv"), header = TRUE, row.names=1) #read in genes-to-module assignments
...
MoverN<-read.csv(here("Data","Input","differential_expression_results_sorted_MNPs_vs_neutrophs.csv"), header=T)# reading in DESeq2 output of neutrophil-MNP differential gene expression
...
#cnetplot
Nt_cnet <- cnetplot(edox_Nt, showCategory=9, foldChange=ordered_Nt)+scale_color_gradientn(colors=coul2)#chose own color gradient
```
...
MoN_dot <- dotplot(edox, showCategory=10) + ggtitle("dotplot for MoverN enriched pathways")+
  scale_color_gradientn(colors=coul)+borderer
MoN_cnet <- cnetplot(edox, showCategory=9, foldChange=ordered_MoN)+scale_color_gradientn(colors=coul2)
